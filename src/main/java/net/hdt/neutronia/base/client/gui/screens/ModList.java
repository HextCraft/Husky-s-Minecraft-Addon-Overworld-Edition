...
public class ModList extends GuiScreen {
     ...
     * @param mainMenu
     */
    public ModList(GuiScreen mainMenu) {
    ...
    private void updateCache() {
        ...
        if (!selectedMod.getMetadata().autogenerated) {
            ...
            lines.add(TextFormatting.GOLD + I18n.format("fml.menu.mods.info.version", selectedMod.getVersion()));
            lines.add(I18n.format("fml.menu.mods.info.idstate", selectedMod.getModId(), Loader.instance().getModState(selectedMod)));

            if (!selectedMod.getMetadata().credits.isEmpty()) {
                lines.add(I18n.format("fml.menu.mods.info.credits", selectedMod.getMetadata().credits));
            }
    ...
    private class Info extends GuiScrollingList {
        ...
        public Info(int width, List<String> lines, @Nullable ResourceLocation logoPath, Dimension logoDims) {
            super(ModList.this.getMinecraftInstance(),
                    width,
                    ModList.this.height,
                    32, ModList.this.height - 88 + 4,
                    ModList.this.listWidth + 20, 60,
                    ModList.this.width,
                    ModList.this.height);
        ...
        private List<ITextComponent> resizeContent(List<String> lines) {
            ...
            for (String line : lines) {
                ...
                if (maxTextLength >= 0) {
                    ret.addAll(GuiUtilRenderComponents.splitText(chat, maxTextLength, ModList.this.fontRenderer, false, true));
                }
            }
            return ret;
        }
        ...
        protected void drawHeader(int entryRight, int relativeY, Tessellator tess) {
            ...
            if (logoPath != null) {
                ...
                ModList.this.mc.renderEngine.bindTexture(logoPath);
            ...
            for (ITextComponent line : lines) {
                if (line != null) {
                    GlStateManager.enableBlend();
                    ModList.this.fontRenderer.drawStringWithShadow(line.getFormattedText(), this.left + 4, top, 0xFFFFFF);
            ...
            if (line != null) {
                ...
                for (ITextComponent part : line) {
                    if (!(part instanceof TextComponentString))
                        continue;
                    k += ModList.this.fontRenderer.getStringWidth(((TextComponentString) part).getText());
                    if (k >= x) {
                        ModList.this.handleComponentClick(part);
                        break;
                    }
                }
            }
