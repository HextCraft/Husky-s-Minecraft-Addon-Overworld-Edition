...
public class CraftingHelper {
    ...
    private static void init()
    {
        ...
        registerC("neutronia_revamped:mod_loaded", (context, json) -> {
        ...
        registerC("neutronia_revamped:and", (context, json) -> {
            JsonArray values = JsonUtils.getJsonArray(json, "values");
            List<BooleanSupplier> children = Lists.newArrayList();
            for (JsonElement j : values)
            {
        ...
        registerC("neutronia_revamped:false", (context, json) -> () -> false);
        ...
        return findFiles(mod, "assets/" + mod.getModId() + "/recipes",
            root ->
            {
                ...
                if (Files.exists(fPath))
                {
                    ...
                    {
                        JsonObject[] json = JsonUtils.fromJson(GSON, reader, JsonObject[].class);
                        ctx.loadConstants(Objects.requireNonNull(json));
                    }
                    catch (IOException e)
                    {
