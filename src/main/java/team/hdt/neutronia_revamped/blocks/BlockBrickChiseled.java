...
public class BlockBrickChiseled extends BlockMod implements INeutroniaBlock {
    ...
    private IBlockState filledBlock;

    public BlockBrickChiseled(String name, boolean filled, IBlockState filledBlock) {
        super(name, Material.ROCK);
        this.filled = filled;
        this.filledBlock = filledBlock;
        setCreativeTab(filled ? null : CreativeTabs.DECORATIONS);
        setHardness(2.0F);
        setResistance(10.0F);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (heldItem == Item.getItemFromBlock(Blocks.STONE_SLAB) && !this.filled) {
            worldIn.setBlockState(pos, filledBlock, 2);
