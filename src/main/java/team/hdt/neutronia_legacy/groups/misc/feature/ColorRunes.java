package team.hdt.neutronia_legacy.groups.misc.feature;
...
import team.hdt.neutronia_legacy.groups.misc.item.ItemRune;
import team.hdt.neutronia_legacy.groups.tweaks.util.ItemNBTHelper;

public class ColorRunes extends Component {

    public static final String TAG_RUNE_ATTACHED = "neutronia_legacy:rune_attached";
    public static final String TAG_RUNE_COLOR = "neutronia_legacy:rune_color";
    ...
    public void onLootTableLoad(LootTableLoadEvent event) {
        ...
        if (event.getName().equals(LootTableList.CHESTS_SIMPLE_DUNGEON))
            event.getTable().getPool("main").addEntry(new LootEntryItem(rune, dungeonWeight, itemQuality, funcs, new LootCondition[0], "neutronia_legacy:rune"));
        else if (event.getName().equals(LootTableList.CHESTS_NETHER_BRIDGE))
            event.getTable().getPool("main").addEntry(new LootEntryItem(rune, netherFortressWeight, itemQuality, funcs, new LootCondition[0], "neutronia_legacy:rune"));
        else if (event.getName().equals(LootTableList.CHESTS_JUNGLE_TEMPLE))
            event.getTable().getPool("main").addEntry(new LootEntryItem(rune, jungleTempleWeight, itemQuality, funcs, new LootCondition[0], "neutronia_legacy:rune"));
        else if (event.getName().equals(LootTableList.CHESTS_DESERT_PYRAMID))
            event.getTable().getPool("main").addEntry(new LootEntryItem(rune, desertTempleWeight, itemQuality, funcs, new LootCondition[0], "neutronia_legacy:rune"));
    }
