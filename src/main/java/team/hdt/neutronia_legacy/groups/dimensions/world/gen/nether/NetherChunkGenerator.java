package team.hdt.neutronia_legacy.groups.dimensions.world.gen.nether;
...
import team.hdt.neutronia_legacy.base.world.biome.BiomeWrapper;
...
public class NetherChunkGenerator extends ChunkGeneratorHell {
                            ...
                            if (checkBlock == Blocks.NETHERRACK.getDefaultState()) {
                                if (i1 == -1) {
                                    if (l <= 0) {
                                        ...
                                        if (RevampedNether.generateGravel && genGravel) {
                                            floorTopBlock = Blocks.GRAVEL.getDefaultState();
                                        }

                                        if (RevampedNether.generateSoulSand && genSoulSand) {
    ...
    public void populate(int chunkX, int chunkZ) {
        ...
        BiomeWrapper wrapper = NNBiomeManager.getBiomeWrapper(world.getBiome(blockPos.add(16, 0, 16)));
    ...
    public List<Biome.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos) {
        ...
        BiomeWrapper wrapper = NNBiomeManager.getBiomeWrapper(world.getBiome(pos));
    ...
    private void replaceBiomeBlocks(int chunkX, int chunkZ, ChunkPrimer primer, Biome[] biomes) {
        ...
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                BiomeWrapper wrapper = NNBiomeManager.getBiomeWrapper(biomes[x + z * 16]);
