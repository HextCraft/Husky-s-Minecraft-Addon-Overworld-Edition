...
public class WorldGenTreeCherry extends WorldGenAbstractTree {
    private static final IBlockState LOG = CherryTrees.cherryLog.getDefaultState();
    private static final IBlockState LEAF = CherryTrees.cherryLeaves.getDefaultState().withProperty(BlockPVJLeaves.CHECK_DECAY, Boolean.FALSE);
    private final boolean useExtraRandomHeight;

    public WorldGenTreeCherry(boolean notify, boolean useExtraRandomHeightIn) {
        super(notify);
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256) {
            ...
            if (!flag) {
                ...
                boolean isSoil = state.getBlock().canSustainPlant(state, worldIn, down, net.minecraft.util.EnumFacing.UP, CherryTrees.cherrySapling);
