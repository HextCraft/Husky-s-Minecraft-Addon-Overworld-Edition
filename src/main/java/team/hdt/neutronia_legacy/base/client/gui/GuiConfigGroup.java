...
import team.hdt.neutronia_legacy.base.client.gui.elements.FeatureSettingsButton;
...
public class GuiConfigGroup extends GuiConfigBase {
    ...
    public void initGui() {
        ...
        title += " - " + I18n.translateToLocal("neutronia_legacy.config.group." + group.name.toLowerCase().replace(" ", "_")) + " (" + components.size() + ")";
...
//        drawRect(0, 0, Minecraft.getMinecraft().displayWidth, 100, Color.LIGHT_GRAY.getRGB());
        ...
        /*for(int i = 0; i < elements.size(); i++) {
            ...
            mc.getTextureManager().bindTexture(new ResourceLocation("neutronia_legacy", "textures/config_gui/test_icon.png"));
            drawModalRectWithCustomSizedTexture(10, 10, 128, 128, 80, 80, 128, 128);
        }*/

        if (mayRequireRestart) {
            String s = I18n.translateToLocal("neutronia_legacy.config.needrestart");
