package team.hdt.neutronia.groups.client.render;
...
import team.hdt.neutronia.groups.client.features.ConsoleHudFeatures;
...
public class RenderSelectedItem extends GuiIngame {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        if (this.mc.isGamePaused() || event.phase != TickEvent.Phase.END || !ConsoleHudFeatures.heldItemTooltips)
            return;
    ...
    public void renderGameOverlayText(RenderGameOverlayEvent.Text event) {
        ...
        } else if (!ConsoleHudFeatures.heldItemTooltips) {
            mc.gameSettings.heldItemTooltips = true;
        }

        if (this.mc.playerController.isSpectator() || !ConsoleHudFeatures.heldItemTooltips) {
            return;
        }
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty()) {
            ...
            j -= ConsoleHudFeatures.heldItemTooltipsBottomMargin % j;
            ...
            if (k > 0) {
                ...
                if (listsize > ConsoleHudFeatures.heldItemTooltipsRows) {
                    listsize = ConsoleHudFeatures.heldItemTooltipsRows;
                }
                if (listsize > 2) {
    ...
    private List<String> getToolTipColour(ItemStack stack) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            } else if (i == ConsoleHudFeatures.heldItemTooltipsRows - 1 && list.size() > ConsoleHudFeatures.heldItemTooltipsRows && ConsoleHudFeatures.heldItemTooltipsDots) {
                list.set(i, TextFormatting.GRAY + "..." + TextFormatting.RESET);
            } else if (stack.getItem() instanceof ItemShulkerBox && list.size() == 7 && i == ConsoleHudFeatures.heldItemTooltipsRows - 1) {
                list.set(i, TextFormatting.GRAY + "" + TextFormatting.ITALIC + TextFormatting.getTextWithoutFormattingCodes(new TextComponentTranslation("container.shulkerBox.more", list.size() - ConsoleHudFeatures.heldItemTooltipsRows + getShulkerBoxExcess(list.get(6))).getFormattedText()) + TextFormatting.RESET);
            } else if (i == ConsoleHudFeatures.heldItemTooltipsRows - 1 && list.size() > ConsoleHudFeatures.heldItemTooltipsRows) {
                list.set(i, TextFormatting.GRAY + "" + TextFormatting.ITALIC + TextFormatting.getTextWithoutFormattingCodes(new TextComponentTranslation("container.shulkerBox.more", list.size() - ConsoleHudFeatures.heldItemTooltipsRows + 1).getFormattedText()) + TextFormatting.RESET);
            } else {
    ...
    private List<String> getTooltip(@Nullable EntityPlayer playerIn, ItemStack stack) {
        ...
        if (ConsoleHudFeatures.heldItemTooltipsModded) {
