...
public class BlockPhantomLantern extends BlockMod implements INeutroniaBlock {

    public static final PropertyDirection FACING = PropertyDirection.create("facing");
    public static final PropertyBool CHAIN_EXTENDED = PropertyBool.create("chain_extended");
    ...
    public BlockPhantomLantern(boolean isOn) {
        super(isOn ? "lit_phantom_lantern" : "phantom_lantern", Material.IRON);
        this.isOn = isOn;
        if (isOn) this.setLightLevel(1.0F);
        setCreativeTab(isOn ? null : CreativeTabs.REDSTONE);
    ...
    public void setOffBlock(IBlockState offBlock) {
        BlockPhantomLantern.offBlock = offBlock;
    }

    public void setOnBlock(IBlockState onBlock) {
        BlockPhantomLantern.onBlock = onBlock;
    }
    ...
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, EnumHand hand) {
        return this.getDefaultState().withProperty(FACING, facing).withProperty(CHAIN_EXTENDED, facing == EnumFacing.UP);
    }
