...
import team.abnormal.neutronia.base.blocks.BlockNeutroniaPlant;
import team.abnormal.neutronia.init.NDamageSource;
import team.abnormal.neutronia.base.blocks.PlantBehaviorType;

import javax.annotation.Nullable;
import java.util.Random;

public class BlockBerryBush extends BlockNeutroniaPlant implements IGrowable {
    ...
    public BlockBerryBush(String berryName, Item berry) {
        super(String.format("%s_bush", berryName), PlantBehaviorType.BUSH);
        this.berry = berry;
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (var5 < 3 && rand.nextInt(5) == 0 && worldIn.getLightFromNeighbors(pos.up()) >= 9) {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (!var11 && playerIn.getHeldItem(hand).getItem() == new ItemStack(Items.DYE, Items.DYE.getMetadata(15)).getItem()) {
            ...
            InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(berry, var12 + (var11 ? 1 : 0)));
