package team.abnormal.neutronia.base.client;
...
public class ContributorRewardHandler implements ISelectiveResourceReloadListener {
    ...
    private static final ImmutableSet<String> MOD_CAPE = ImmutableSet.of("b344687b-ec74-479a-9540-1aa8ccb13e92",
    ...
    public static void onRenderPlayerPost(RenderPlayerEvent.Post event) {
        ...
        NetworkPlayerInfo info = ReflectionHelper.getPrivateValue(AbstractClientPlayer.class, clplayer, NObfuscations.PLAYER_INFO);
        Map<Type, ResourceLocation> textures = ReflectionHelper.getPrivateValue(NetworkPlayerInfo.class, info, NObfuscations.PLAYER_TEXTURES);
        if (!done.contains(player) && clplayer.hasPlayerInfo()) {
            if (MOD_CAPE.contains(uuid)) {
                ResourceLocation loc = new ResourceLocation(Reference.MOD_ID, "textures/misc/mod_cape.png");
                textures.put(Type.CAPE, loc);
                textures.put(Type.ELYTRA, loc);
                done.add(player);
            }
            if (TEAM_CAPE.contains(uuid)) {
                ResourceLocation loc = new ResourceLocation(Reference.MOD_ID, "textures/misc/team_cape.png");
                textures.put(Type.CAPE, loc);
                textures.put(Type.ELYTRA, loc);
                done.add(player);
            }
            if (MCA_CAPE.contains(uuid)) {
                ResourceLocation loc = new ResourceLocation(Reference.MOD_ID, "textures/misc/mca_cape.png");
                textures.put(Type.CAPE, loc);
                textures.put(Type.ELYTRA, loc);
                done.add(player);
            }
