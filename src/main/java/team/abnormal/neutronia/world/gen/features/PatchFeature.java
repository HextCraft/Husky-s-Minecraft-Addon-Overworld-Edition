...
public class PatchFeature extends WorldGenerator {
    private final IBlockState state;
    private final int size;
    private List<Block> replaceableBlocks = new ArrayList<>();

    public PatchFeature(IBlockState state, int size, Block... replaceableBlocks) {
        this.state = state;
        this.size = size;
        this.replaceableBlocks.addAll(Arrays.asList(replaceableBlocks));
    }

    public PatchFeature(IBlockState state, int size) {
